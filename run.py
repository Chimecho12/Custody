"""itx 명령줄 진입점.
  ...
  python run.py doctor            환경 점검 (Python 버전, 서명 백엔드)
  python run.py test              단위 테스트
  python run.py run [--out DIR]   시나리오 17종 × 모드 3종 실행 + Q1 매트릭스 → artifacts/
  python run.py report [--out DIR] artifacts/results.json → artifacts/report.html
  python run.py audit [--out DIR] artifacts/log-export-S01.json 을 독립 재실행해 판정·앵커 검증
  python run.py all               run + report + audit
"""
...
def main(argv: list[str] | None = None) -> int:
    for stream in (sys.stdout, sys.stderr):  # Windows 콘솔·파이프에서 한글이 깨지지 않게
        try:
