...
const ANIM_NOTES = [
  {title: '요청의 이동과 진술 발행', spec: '사건 상세 재생 · 실제 홉 지연 비례', impl: true,
   body: 'U→R→M→R→U 를 점 하나가 지난다. 구간 경계는 임의 데모 수치가 아니라 이 시도의 실제 sent_at/received_at 과 시뮬레이션 지연 상수(홉 20ms·중개 처리 5ms·서명 2ms)에서 역산한 것이다. 이동 구간은 정지에서 출발해 정지로 끝나므로 가감속을 주고, 꺾은선의 꼭짓점은 엣지와 같은 반지름(14px)으로 둥글려 선 위를 실제로 타고 돈다. 재생 벽시계는 구간마다 다르다 — 20ms 홉은 눈이 궤적을 쫓도록 최소 480ms, 200ms 추론은 멈춘 것처럼 보이지 않게 최대 750ms. 표시되는 t 와 눈금은 언제나 실측 그대로다.'},
  ...
  {title: '노드 도달 펄스 · 흡수 · 호흡', spec: '사건 상세 재생 · 1회성 fade + 재생 중 호흡', impl: true,
   body: '패킷이 U/R/M 상자에 닿는 순간 그 상자 바깥에만 4px 링이 300ms 동안 잦아들고, 상자가 1.5% 부풀었다 돌아온다(흡수). 상자가 패킷을 안고 일하는 동안(R 처리·M 추론·서명)은 테두리가 1.2초 주기로 숨 쉰다 — 멈춘 것이 아니라 연산 중이라는 뜻이며 재생 중에만 붙는다. 반복 점멸하지 않고 잔상도 남기지 않는다. 뒤로 이동하면 다시 낼 수 있게 초기화되고, 선택을 바꿔 최종 상태로 들어올 때는 내지 않는다 — 방금 일어난 일이 아니기 때문이다.'},
  {title: '변조의 순간', spec: '사건 상세 재생 · 색 전이 + 파문 1회', impl: true,
   body: '요청 변조(E4 실패)는 R→M 구간에서, 응답 변조(E10 실패)는 M→R 구간부터 점·발광·꼬리의 색이 파랑에서 빨강으로 바뀐다. 그 프레임에 붉은 파문 하나와 240ms 의 흔들림을 딱 한 번 내고, 이후에는 색만 남긴다 — 계속 떠는 점은 인과가 아니라 소음이다. 실제로 실패한 등식에서 색을 가져오므로 시나리오마다 자동으로 맞다.'},
