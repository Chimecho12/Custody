...
import {
  Data, esc, short, CV, hopMapHtml, checkLabels, checkEdges, eqLabels, eqEdges, idleEdges, CHECK_POS, legendRowHtml,
  playControlsHtml, computeLegs, makeSpan, PlayContext, timeBoxHtml, Mark, evidenceRowsHtml, stripGridHtml,
  checksTableHtml, equationTableHtml, codesHtml, gateColor, checkChipsHtml, st,
} from './console';
...
export interface RouteCard { html: string; ctx: PlayContext | null }

// 실행 전 상태: 모든 검사가 회색이다. 부재를 사건처럼 그리지 않는다.
export function idleRouteHtml(status: Data | null): string {
  const labels = CHECK_POS.map(([id, x, y, anchor, tail]) => ({id, x, y, anchor, tail, result: 'not_evaluable', big: id === 'response_binding'}));
  return `<div class="card-head"><h3>경로 — 업무 데이터 경로(실선)와 T 의 증거·통제 경로(점선)</h3><div class="badges"><span>${status ? esc(status.source) : '연결 전'}</span><span>evaluation</span></div></div>
    ${hopMapHtml('req', labels, idleEdges, '홉 정합 지도 — 실행 전', roles(status))}
    ${legendRowHtml('<span class="muted">아직 실행한 요청이 없다. 첫 요청 뒤 U 의 로컬 검사 결과가 이 지도에 표시된다.</span>')}`;
}
function roles(status: Data | null) {
  return {u: '사용자 · 집행 모듈', r: '중개자 (클라우드 대행)', m: status?.model_kind === 'deterministic_mock' ? '모델 운영자 · 결정적 모형' : '모델 운영자 · 실제 모델 어댑터', t: 'T — 독립 검증 역할', tSub: '대조 · 정책 · 추가 전용 로그'};
}
...
export function routeCard(record: Data, status: Data | null, mode: MapMode): RouteCard {
  ...
  const labels = useEq ? eqLabels(v.equations) : checkLabels(checks);
  const edges = useEq ? eqEdges(v.equations) : checkEdges(checks);

  let ctx: PlayContext | null = null, timeHtml = '';
  if (mm.sent !== null && received > sent) {
    const legs = computeLegs(sent, received, 200);
    ...
    const marks: Mark[] = [
      {label: '요청 전송', at: `${sent} ms`, pos: span(sent), color: 'muted'},
      {label: mm.received === null ? '응답 없음' : 'U 수신', at: `${received} ms`, pos: span(received), color: 'muted'},
    ];
    if (decided !== null) marks.push({label: '게이트 결정', at: `${decided} ms`, pos: span(decided), color: 'pass'});
    if (released !== null) marks.push({label: '응답 공개', at: `${released} ms`, pos: span(released), color: harmExposed ? 'fail' : 'muted'});
    else if (mm.quarantined !== null) marks.push({label: '응답 격리', at: 'null (미공개)', pos: span(mm.quarantined) + 2, color: 'na'});
    else if (mm.aborted !== null) marks.push({label: '중단', at: `${mm.aborted} ms`, pos: span(mm.aborted), color: 'na'});
    if (verdictAt !== null) marks.push({label: 'T 판정 발행', at: `${verdictAt} ms · T 시계`, pos: span(verdictAt), color: 'pass'});
    else if (v) marks.push({label: 'T 판정', at: '사후 조회', pos: 96, color: 'na'});
    timeHtml = `<h3>시점 — 검증 전에 무엇이 공개되었는가</h3>${timeBoxHtml(marks)}
      <p class="small muted" style="margin-top:6px">${timeNote(record, mm, g, decided, released, verdictAt, attack)}</p>`;
  } else {
    timeHtml = `<h3>시점</h3><p class="small muted">전송 전에 중단되어 시점 축이 없다.</p>`;
  }
  const evidence = evidenceRowsHtml([
    {key: 'U', name: 'U 요청 계약', detail: 'contract · 허용 모델·변환·만료 서명 · 전송 전 큐 등록', reg: null, present: true},
    {key: 'R', name: 'R 중계 진술', detail: 'relay · 응답과 함께 동봉', reg: null, present: checks.R_authority?.result === 'pass'},
    {key: 'M', name: 'M 추론 영수증', detail: 'receipt · 요청/응답 커밋, eat_nonce, model_id', reg: null, present: checks.M_authority?.result === 'pass'},
    {key: 'O', name: 'U 수신 진술', detail: 'observation · 실제 받은 응답의 커밋', reg: null, present: mm.received !== null},
  ]);
  ...
  const html = `<div class="card-head"><h3>경로 — 업무 데이터 경로(실선)와 T 의 증거·통제 경로(점선)</h3>
    ...
    ${hopMapHtml('req', labels, edges, '홉 정합 지도 — ' + record.sub, roles(status))}
    ...
    ${stripGridHtml([
      ...
      {k: '정책 세대', v: status?.epoch != null ? esc(String(status.epoch)) : '실험실'}, {k: '집행 모드', v: esc(record.mode)}])}`;
  return {html, ctx};
}
