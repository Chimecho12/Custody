...
- 첫 버전은 파일 기반 목격자 모사다. 인터페이스만 고정하고 실제 배포에서는
  RFC 3161 타임스탬프 서버, Sigstore Rekor, 공개 체인 테스트넷 중 하나로 교체한다.
...
class CheckpointAnchor:
    ...
    @staticmethod
    def verify_tree(tree: MerkleTree, records: list[dict[str, Any]]) -> list[dict[str, Any]]:
