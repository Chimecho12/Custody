...
큐 기본 한도는 256건이고 기존 증거를 밀어내지 않는다. 한도를 넘긴 뒤의 동작은 정책이다(`evidence_backlog_policy`): 기본 `bounded` 는 새 요청을 거부하고, `unbounded` 는 요청 보관 한도까지 계속 쌓는다.
R 이 중계 진술을 동봉하지 않으면 결손(not_evaluable)으로 기록하고 U+M 증거로 계속한다.
`python runtime.py endurance` 는 T 정지·복구를 낀 연속 요청(기본 24건, 정지 구간 8~16)을 protect·strict 로 돌려 구간별 서비스 여부·적체 크기·복구 뒤 적체가 비는 시간·모든 요청의 사후 판정 여부·복구 뒤 감사 결과를 남긴다. 단기 표본(benchmark)이 답하지 못한 "장애를 지나 계속 쓸 수 있는가" 를 루프백에서 실측한 것이며, 원격 운영자의 장애·부하는 아니다.
U·R·M 은 T 의 등록 영수증을 검증 후 각자 보관한다. 감사 시 Agent 는 R·M 에게 `held_receipts` 를 요청해 세 당사자의 영수증으로 포함 검사를 한다. `require_relay_statement: true` 일 때만 격리한다.
