...
python run.py all        # 시나리오 17종 × 모드 3종 실행 → artifacts/results.json, report.html, 독립 감사
```
...
| T 검증 | 감사자 | 로그 내보내기·앵커만으로 트리·판정을 재실행. T 오판·기록 재작성 발견 | `itx/audit/replay.py`, `itx/ts/anchor.py` |

T 의 신용은 "믿게 만들기" 가 아니라 **틀리거나 침해되어도 드러나고 교체할 수 있게 만들기** 로
확보한다. 시나리오 S13(T 오판)·S14(T 기록 재작성)이 이를 시험한다. U 는 T 가 '통과' 라고
해도 로컬 검증이 실패하면 격리한다.
...
S10(T 정지)·S11(등록 지연)·S12(큐 포화)가 각 모드의 비용을 실측(모의)한다.
...
| S09 | R 비협조 | passed 이지만 completeness=gap. 가해자 단정 없음 |
